...
| SITE-BUILD-14 | The hub, plugin, and tutorial pages are pure build artifacts: their hand-edited SOURCE lives under `site-src/pages/` and `build_site_data.py` assembles the committed `site/**` pages from it, injecting a `GENERATED FILE - DO NOT EDIT` banner (and a `/* ... */` banner in `styles.css`) that names the source. Because the source is independent of the artifact, `--check` compares the whole built page (not just the marker regions) to the committed file, so a hand-edit to any part of a built page fails `--check`; a page whose source was removed but whose built artifact lingers is flagged as an orphan by `--check` and deleted on a normal build. | `scripts/test_build_site_data.py` - `CheckDriftTests.test_check_flags_a_hand_edited_built_page`, `CheckDriftTests.test_check_flags_an_orphaned_page_whose_source_was_removed`, `CheckDriftTests.test_write_removes_an_orphaned_page_whose_source_was_removed` (end-to-end via `main(["--check"])`); `PageBannerAndGuardTests` (banner present/idempotent, `build_page` fills+banners, hand-edit differs from a fresh build, missing artifact is drift) |
