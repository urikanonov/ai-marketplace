...
| SITE-NAV-01 | Every section with an `id` and a `.section-title` heading (on the hub and plugin pages) gets a `.header-anchor` link appended to the heading with `href="#<section-id>"`, so a reader can link directly to that section. Clicking the anchor updates the URL fragment (and copies the section's full URL to the clipboard when available). The anchor is a progressive enhancement wired by `site.js`. | `tests/site/tests/site.spec.js` - `every section header is a linkable anchor that updates the URL fragment (SITE-NAV-01)` |
