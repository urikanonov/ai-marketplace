...
- A generated document no longer embeds the vendored rich libraries. It carried mermaid and Chart.js
  as a 1,392,096-byte (1,359 KiB) gzip+base64 payload that the VIEWER never read - the viewer imports
  mermaid from the CDN, and an authored Chart.js arrives as its own CDN script with an SRI hash - so
  the only consumer was the Offline export, and every reader paid over a megabyte to pre-stage an
  export most of them never run. The payload is now a byte-free descriptor of 2,629 bytes: the pinned
  CDN URL for each library plus the SHA-384 hash recorded at build time, with the MIT notices still
  embedded so compliance never depends on the network. Measured on the shipped examples:
  `report-metrics.html` 2,217,964 -> 831,745 bytes (-62%), the four diagram-carrying examples each
  drop 57 to 62 percent, and the whole example corpus drops 47 percent (12,035,861 -> 6,406,508
  bytes).
- Export Offline downloads the pinned library it needs and VERIFIES it against that recorded hash
  before inlining it, exactly where it used to inflate the embedded bytes. The export therefore needs
  network access once; the file it produces is unchanged - still fully self-contained, still under
  the same zero-network CSP. It fetches the UMD build rather than the ESM entry the viewer uses,
  because the exporter drives `window.mermaid` as a classic global and a UMD file is self-contained,
  whereas the ESM entry would still lazy-load its per-diagram chunks. Nothing is downloaded while
  merely reading a document, and a library the document cannot use is never downloaded, decoded, or
  allowed to fail an export.
- Every way the download can go wrong fails CLOSED and says so, with no file written: a blocked or
  failed request, bytes that do not match the recorded hash, a hash that is malformed, a recorded URL
  that is not https, a redirect that lands on a non-https address, a page that is not a secure
  context (the message names the fix - open the file locally or serve it over https), and a
  connection that is accepted and then never answers. A payload that names a library with only half
  a descriptor - a URL and no hash, or a hash and no URL - is refused too, rather than being read as
  "this document does not carry the library" and quietly falling through to the document's own
  unverified copy.
- Documents generated by an earlier version are unaffected: a payload that still carries bytes is
  honoured exactly as before, with no network access at all.
- The export sends no referrer to the CDN, so downloading a library never discloses which document is
  being exported. The two downloads share one time budget rather than one each.
- `tools/build.py --vendor-bytes` is a troubleshooting escape hatch that restores the old
  self-contained form, for an author who must export with no network. It is never used by the
  committed build, so it is refused alongside `--check` and does not enforce the size budget.
- The `dist/SHAREABLE.html` and `dist/NONSHAREABLE.html` size-budget ceilings drop to match, so the
  win cannot silently drift back.

### Fixed

- The vendored Chart.js copy is now held to the same provenance guarantee as mermaid: its version
  pin, the bytes in `assets/vendor/chart.umd.min.js`, and the installed npm tarball must all agree.
  Without that, a lockfile-only bump followed by a re-vendor could record a URL and a hash for
  different releases, and every chart document's offline export would fail verification permanently.
- A document carrying two vendored payload blocks no longer merges them into one that holds a stale
  library's bytes beside a newer one's descriptor. Only one form can ever be used, and the bytes win,
  so the later block silently lost and a right-sized document could be re-inflated.
...
- A finished document no longer stores a checklist item's identity twice. A survey of 15 generated
  documents (#1250) found their bodies compress 18.7x because a tree of about 1700 rows repeats the
  same values on every row, and two attributes held the byte-identical string on all 1725 items of
  one document: `data-cmh-item` and an authored `data-<ns>-id` (about 168 KB duplicated in a single
  file, and the same again between `data-cmh-parent` and `data-<ns>-parent`). `finalize` now runs a
  last trim (`tools/authoring/dom_slim.py`, on by default, `--no-slim` opts out) that drops the
  `data-cmh-*` copy and records where to read it from in `data-cmh-item-attr` /
  `data-cmh-parent-attr` on the container; the runtime, the validator and `checklist_apply.py` all
  DERIVE the id from there, preferring the item's own attribute while it is still present.
- Nothing a reader can observe changes. A document generated by an earlier version opens, comments,
  exports and reopens byte-for-byte as it always did, a trimmed document resolves the identical item
  keys - so comment threads, handled-comment ids, reviewed-section state and checklist storage carry
  over with zero data loss - a rendered-DOM snapshot of a trimmed document matches its untrimmed
  twin, and keyboard cycling and the announced state are unchanged.
- The dedupe only fires when it can prove the identity is genuinely the same one AND that the
  document can still be read. It is all-or-nothing per checklist (every row and the container are
  proved rewritable before anything is mutated); ownership matches the runtime exactly, decided by
  attribute presence and ancestor-or-self, so an empty `data-cmh-checklist=""` is still a boundary
  and a nested container is never counted as an item of the list around it; ownership must also be
  provable on a tag-nesting tree, so a container a browser would have closed early (a `<p>` holding
  block children) is refused rather than guessed at; and the document's own embedded runtime must
  demonstrably read the pointer, proved by an executable script outside the content region. That
  last gate leaves a NonShareable document alone, since its runtime is a sibling file the tools can
  neither see nor version and a stale companion would fall back to positional keys and orphan saved
  checklist state.
- Two further transforms were implemented and then removed rather than shipped, because each was
  measured at zero effect on every document this repository ships while changing something a reader
  can observe. Hoisting a shared child class onto its container could not be made sound by a static
  tool (a rule can live in a linked or imported stylesheet it never sees, a generic `[class]`
  selector stops matching either way, a CSS-escaped spelling evades a token scan, and a
  child-combinator selector matches children added later) and it weakened an enforcement layer: the
  WCAG contrast checker refuses any selector carrying a combinator, so the rewritten rule became
  invisible to it while the original matched nothing. Dropping an `aria-label` equal to the
  element's text foundered on the accessible-name computation: a name falls back to content only
  for the roles that support it, and measured through Chromium, stripping the label emptied the
  accessible name on `li`, `p`, `ul`, `dd`, `blockquote`, `label`, `tr`, `details`, `div` and
  `span`, as well as on landmarks and headings. `title` is not trimmed either - it is redundant
  only while the whole text is on screen, and under CSS truncation it is the only way to read the
  rest.
## [1.832.0] - 2026-08-10
...
- The generated table of contents now sits BELOW the document title and its reading-time strip
  ...
  than duplicated, and re-running the tool is idempotent for the shapes above. The nav keeps its
  top-of-root placement wherever anchoring under the title would be wrong or unsafe: a document
  with no `<h1>`, one whose title container was never closed by its own end tag, one whose title
  only exists inside the nav being replaced, one where a listed heading sits inside the title
  container or before the title (a slide deck, a single-wrapper document, a lede carrying a listed
  subtitle), and one with no sections for the nav to list at all.
