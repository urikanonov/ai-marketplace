...
- A generated document no longer embeds the vendored rich libraries. It carried mermaid and Chart.js
  as about 1,357 KB of gzip+base64 that the VIEWER never read - the viewer imports mermaid from the
  CDN, and an authored Chart.js arrives as its own CDN script with an SRI hash - so the only consumer
  was the Offline export, and every reader paid a megabyte to pre-stage an export most of them never
  run. The payload is now a byte-free descriptor of about 2.5 KB: the pinned CDN URL for each library
  plus the SHA-384 hash recorded at build time, with the MIT notices still embedded so compliance
  never depends on the network. Measured on the shipped examples: `report-metrics.html` 2,166 KB ->
  811 KB, and the four diagram-carrying examples each drop 57 to 63 percent.
- Export Offline downloads the pinned library it needs and VERIFIES it against that recorded hash
  ...
  merely reading a document, and nothing is downloaded for a library the document cannot use.
- Every way the download can go wrong fails CLOSED and says so, with no file written: a blocked or
  failed request, bytes that do not match the recorded hash, a hash that is malformed, and a recorded
  URL that is not https each produce a specific, actionable error rather than a silent download whose
  diagrams would never render.
