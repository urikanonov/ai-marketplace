#!/usr/bin/env python3
"""Tests for tools/validate/cmhval/contrast.py (CMH-DECK-12)."""
import os
import sys
import unittest
...
class ContrastUtilityTests(unittest.TestCase):
    def test_cmh_deck_12_black_white_ratio_is_21_to_1(self):
    ...
    def test_cmh_deck_12_equal_colors_ratio_is_1_to_1(self):
        self.assertAlmostEqual(contrast.contrast_ratio("#123456", "#123456"), 1.0, places=2)

    def test_cmh_deck_12_parser_supports_hex_rgb_rgba_and_vars(self):
    ...
    def test_cmh_deck_12_finds_configured_variable_pairs(self):
    ...
    def test_cmh_deck_12_skips_semi_transparent_backgrounds(self):
        html = """
    ...
    def test_cmh_deck_12_background_shorthand_uses_declaration_order(self):
        html = "<style>.ordered { color:#fff; background-color:#000; background:#eee; }</style>"
        issues = contrast.find_low_contrast_pairs(html)
        self.assertEqual(len(issues), 1)
        self.assertEqual(issues[0].background, "#eee")

    def test_cmh_deck_12_background_url_text_is_not_a_color(self):
        html = """
        <style>
