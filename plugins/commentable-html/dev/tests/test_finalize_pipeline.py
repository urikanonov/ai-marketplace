...
def _legacy_head_payload(html):
    """The document as a PRE-CMH-SIZE-02 file: the vendored payload back in the `<head>`.

    Since the shell became content-first (CMH-SIZE-02) it already parks the payload where the
    relocation converges, so a FRESH document makes `_apply_vendored_libs` a no-op - and this
    fixture exists precisely to keep every always-on phase non-trivial, so the equivalence test
    above is never proven on a pipeline of no-ops. Feeding it the legacy shape restores the
    phase's work AND pins that finalize still repairs a document generated by an older version.
    """
...
class _Case(unittest.TestCase):
    def setUp(self):
        ...
        _write(self.doc, _legacy_head_payload(new_document.make_document(
