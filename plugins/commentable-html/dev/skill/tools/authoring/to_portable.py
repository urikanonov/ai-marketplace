#!/usr/bin/env python3
"""Migrate an existing NonPortable commentable-html document to a self-contained Portable one.

Portable is the ONLY mode this skill now GENERATES, but NonPortable documents created by earlier
releases must keep working indefinitely - there is no deprecation deadline, and the NonPortable
runtime and its companion files stay shipped for exactly that reason. This tool is the migration
path for an author who WANTS an existing NonPortable document to become self-contained: it inlines
the layer so the file no longer depends on `commentable-html.{css,js,assets.js}` sitting beside it.

It is the counterpart to `upgrade.py`, which deliberately REFUSES a NonPortable document (a
NonPortable file is upgraded by replacing its companions, not by swapping inline regions). Run
this first if you want such a document to become Portable, then `upgrade.py` keeps it current.
...
Usage:
  python tools/authoring/to_portable.py <file.html> [more.html ...]
  python tools/authoring/to_portable.py --check <file.html>     # report, write nothing
"""
import argparse
import io
import json
...
# The companion files a NonPortable document references, and the element each is inlined into.
...
# turn a NonPortable document into a Portable one without fetch() (which file:// blocks). Once
# this tool has done that conversion the registry is dead weight - a natively generated
# dist/PORTABLE.html does not carry it either - so migrating drops its reference instead of
# inlining 937 KB of duplicated payload.
DROPPED_COMPANION = "commentable-html.assets.js"

_BOOTSTRAP_BEGIN = "<!-- BEGIN: commentable-html - NONPORTABLE BOOTSTRAP -->"
_BOOTSTRAP_END = "<!-- END: commentable-html - NONPORTABLE BOOTSTRAP -->"
...
def _strip_bootstrap_edit(html, span):
    """The (start, end, "") edit removing the NonPortable bootstrap block, or None.

    Anchored on the exact BEGIN/END comment pair and only OUTSIDE the authored CONTENT region:
    authored content can and does keep a literal, unescaped `NONPORTABLE BOOTSTRAP` anchor pair
    ...
    """
    start = -1
    for m in re.finditer(re.escape(_BOOTSTRAP_BEGIN), html):
        if m.start() < span[0] or m.start() >= span[1]:
            start = m.start()
            break
    if start == -1:
        return None
    end = html.find(_BOOTSTRAP_END, start)
    if end == -1 or (span[0] <= end < span[1]):
        return None
    return (start, _consume_eol(html, end + len(_BOOTSTRAP_END)), "")


_LOADER_NOTE = "<!-- commentable-html - layer loaded from companion files (nonportable mode) -->"
...
def _loader_note_edits(html, span):
    ...
    return [(m.start(), _consume_eol(html, m.end()), "")
            for m in re.finditer(re.escape(_LOADER_NOTE), html)
...
_DESCRIPTOR_RE = re.compile(
    ...
    re.IGNORECASE)
_MODE_VALUE_RE = re.compile(r'("mode"\s*:\s*)"nonportable"')


def _mode_edit(html, masked, target_name):
    """The edit flipping the layer descriptor's mode to portable, or None when already portable.
    ...
    a document that was still marked nonportable. So: parse the descriptor, require exactly one
    ...
    try:
        ...
        raise ValueError("%s has an unparsable layer descriptor; refusing to migrate it"
                         % target_name)
    if descriptor.get("mode") == "portable":
        return None
    if descriptor.get("mode") != "nonportable":
        raise ValueError("%s declares an unknown layer mode %r; refusing to migrate it"
                         % (target_name, descriptor.get("mode")))
    swapped, count = _MODE_VALUE_RE.subn(r'\1"portable"', body)
    if count != 1:
        raise ValueError("%s: expected exactly one descriptor mode to switch, found %d"
                         % (target_name, count))
    if json.loads(swapped).get("mode") != "portable":
        raise ValueError("%s: the descriptor did not end up portable; refusing to write"
                         % target_name)
    ...
    if not found:
        if required:
            raise ValueError(
                ...
                "NonPortable document built by this skill" % (target_name, name))
        return None
    if len(found) > 1:
...
def _neutralize_raw_text_close(text, tag):
    ...
    comments. Escaping the slash is what the runtime's own Export-as-Portable path does: it
...
def _inlined_element(name, tag, body, target_name):
    ...
    Inlining the COMPANION FILES is deliberate; copying the Portable template's CSS/JS region
...
def _assert_self_contained(html, span, target_name):
    ...
    The validator calls a document NonPortable if ANY companion reference survives, so a
    leftover one (an unusual element shape this tool did not rewrite) would produce a file that
    both claims portable and still needs its companions - broken, and written over the user's
...
def to_portable(target_html, layer, target_name="<target>"):
    """Return (new_html, changed). Raises ValueError on an unusable input.

    `layer` maps each companion file name to its text (see `read_layer`). An ALREADY-Portable
    ...
    for marker in upgrade.REQUIRED_MARKERS:
        ...
        if not found:
            raise ValueError(
                ...
                % (target_name, marker))
    if not is_nonportable(target_html):
    ...
    edits.append(_companion_edit(
        ...
        _inlined_element("commentable-html.js", "script", layer["commentable-html.js"],
                         target_name), target_name))
    # The asset registry is OPTIONAL: the validator only warns when a NonPortable document has
...
def main(argv):
    parser = argparse.ArgumentParser(
        prog="to_portable.py",
        description="Migrate a NonPortable commentable-html document to a self-contained "
                    "Portable one, preserving its content, embedded comments, and handled ids. "
                    "An already-Portable document is left untouched.")
    ...
    for path in args.files:
        try:
            ...
            sys.stderr.write("to_portable: %s: %s\n" % (path, exc))
            failed = True
            continue
        try:
            out, changed = to_portable(html, layer, path)
        except ValueError as exc:
            sys.stderr.write("to_portable: %s\n" % exc)
            failed = True
            continue
        if not changed:
            print("to_portable: %s is already Portable; nothing to do" % path)
            continue
        if args.check:
            print("to_portable: %s WOULD be migrated to Portable" % path)
            continue
        try:
            _write(path, out)
        except (OSError, IOError) as exc:
            sys.stderr.write("to_portable: %s: could not write the migrated document: %s\n"
                             % (path, exc))
            failed = True
            continue
        print("to_portable: migrated %s to Portable (%d -> %d bytes)"
