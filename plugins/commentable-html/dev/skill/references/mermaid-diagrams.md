...
The shipped helper is robust by construction: a centred FLEX-WRAP of content-HUGGING framed cards, each rendering its diagram at one uniform HEIGHT. Every diagram is rendered at one fixed HEIGHT with its WIDTH derived from the mermaid viewBox aspect ratio (at its natural, readable size), and each card SHRINKS to hug that width up to a generous cap - so every diagram fills its card in both dimensions: a tall-narrow diagram gets a narrow full-height card, a wide diagram gets a wide card, and a small diagram is scaled UP to the fixed height (never left tiny, marooned, or slivered). (A captioned `<figure>` card is that uniform diagram height PLUS its caption, so a captioned card's TOTAL height varies with its caption; only the diagram area is uniform.) A diagram wider than the cap keeps its full readable height and scrolls horizontally inside its card, instead of being crushed to an unreadable strip. Sizing is pure deterministic CSS anchored on a definite height, so it renders consistently (geometry-identical) across Chromium, Firefox, and WebKit and has no JS-measurement race. On the framed desktop layout the layer's own narrow/wide scale-up is additionally DISABLED for a gallery diagram (it is measurement-timing dependent and rendered diagrams tiny in a real browser). On phones (below the 481px breakpoint) the framing does not apply - the gallery collapses to a single-column flow and a wide diagram keeps the layer's normal wide-scroll handling - and print stacks it one per column at natural size.
