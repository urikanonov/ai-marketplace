...
It prints one `ERROR` / `WARNING` line per issue. By default it exits `0` when every file passes (warnings allowed) and `1` when any file has errors; `--strict` also fails on any warning, so a single run surfaces *everything* to fix and you iterate until it reports `OK (0 warning(s))`. The one exception is an **advisory** warning - a line beginning `code highlighting advisory:` - which names something you cannot clear (a deliberately hand-written code block that the authoring tools pass through verbatim), so it is reported but never fails `--strict`, never blocks `retrofit.py` / `content_replace.py` / `upgrade.py --strict`, and never withholds the validated stamp. A document whose only remaining warnings are advisories is done. (The separate `theme contrast advisory:` lines are NOT in that class - a near-miss ships a concrete `--suggest` fix, so it is clearable and still fails `--strict`; only `retrofit.py` carves it out.) When the document embeds Chart.js charts (a `<canvas>` is present), it **also** runs the chart-embedding checks automatically (see `charts-embedding.md`); pass `--charts-only` or `--layer-only` to run just one half. If Python is not installed, skip it and fall back to the manual [Quick verification after retrofitting](#quick-verification-after-retrofitting) checks.
...
**Offline-mode errors (only when the layer descriptor's mode is `offline`):** an offline document promises that opening it makes no network request at all, so these are checked in addition to everything above. Every network-loading resource is an error with no Chart.js exemption, and the restrictive offline CSP meta is required. Two rules are about the shape, not the URL, because the Offline export removes them whatever they point at - so a file that keeps one is a file an export would change: **every** `<meta http-equiv="refresh">` is an error (a relative target too - a refresh is a top-level navigation no meta-delivered policy can restrict), and **every** `on*` attribute is an error, template content and `<noscript>` fallback markup included. The `on*` test is the export's own literal `^on`, so `once`/`onward` are rejected as well; rename such an attribute (for example to `data-once`) rather than expecting the gate to be cleverer than the strip that removes it.
...
- A `cmh-kql-run` ("Run in Azure Data Explorer") link **outside a framed figure** does not point at `https://dataexplorer.azure.com/`, or uses `target="_blank"` without `rel="noopener"`. (Inside a framed `figure.cmh-kql` a non-ADX run-link href is a hard error - see above.)
