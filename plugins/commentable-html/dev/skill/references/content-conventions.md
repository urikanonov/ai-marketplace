...
**Put a per-document `<style>` in the style slot, not in `<head>`.** A document generated by the skill puts the authored content FIRST and every generated block - the layer stylesheet included - after it, inside one `BEGIN/END: commentable-html - MACHINERY` fence (CMH-SIZE-02). Source order decides an equal-specificity tie, so a `<style>` left in `<head>` now loses to the layer's own `:root` block. The generated template carries a marked slot for exactly this, immediately after the layer stylesheet inside the fence:

```html
<!-- Per-document style slot. ... -->
<style>
  #commentRoot .my-grid { display: grid; }
</style>
```

For brand tokens (`--cp-*`) prefer `--brand <profile.json>`, which the tooling places in the right slot for you. And do not measure layout from an authored inline script at PARSE time - the stylesheet has not been parsed yet at that point; do it on `DOMContentLoaded`.
