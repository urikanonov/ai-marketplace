...
The `<script id="embeddedComments">` block is the **only** comment-state part of the file that **Export as Portable** rewrites (aside from inlining the layer in nonportable mode, below). It leaves the `commentable-html-session-id` and `commentable-html-agent` provenance meta tags in place. It does not regenerate any other markup, does not re-run mermaid, does not change `handledCommentIds`, and does not modify the five pasteable regions.
...
The overflow menu and sidebar expose an **Export Offline** button for handoff situations where the recipient may have no network. It is an addition to **Export as Portable**; Portable, Offline, and Plain HTML exports all keep the session-id/agent provenance meta, while Markdown export is unchanged.
