...
- [Upgrade an existing instance to a new dist/SHAREABLE.html](#upgrade-an-existing-instance-to-a-new-distshareablehtml)
...
The retrofitted document is Shareable (the layer inlined into one file), like every other creation route; `--shareable`
is accepted and simply names that default. A legacy NonShareable retrofit is an explicit request: pass `--nonshareable`,
...
by hand. The five `BEGIN/END: commentable-html - <REGION>` blocks in `dist/SHAREABLE.html` are designed to be copied
verbatim into another document:

1. **Open `dist/SHAREABLE.html`** and locate the five region pairs. Each region is one contiguous block bracketed by
...
4. **Paste EMBEDDED COMMENTS region** immediately after HANDLED IDS. Leave its payload as `[]`; **Export as Shareable**
   writes into it.
...
8. **Avoid the duplicate-root footgun.** `dist/SHAREABLE.html` contains a commented example `#commentRoot` with key
...
## Upgrade an existing instance to a new dist/SHAREABLE.html
...
2. **For CSS, COMMENT UI, and JS:** delete everything between the markers (markers inclusive) and replace it with the same region from the new `dist/SHAREABLE.html`. These three regions are byte-identical across deployments.
...
Because an upgrade never rewrites `#commentRoot`, a `nav.cm-toc` a PRE-1.830 generator produced keeps its
...
region marker in the file. `dist/SHAREABLE.html` contains earlier marker-like strings inside the JS body, so
...
- `window.__COMMENTABLE_ASSETS__` (object `{ version, css, js }` or absent) - present only in the non-shareable (companion `commentable-html.assets.js`) shape; it carries the layer CSS/JS strings that **Export as Shareable/Standalone** inlines. Its `version` must equal `__commentableHtmlVersion`; a mismatch aborts a standalone export by design.