...
export actions, **Manage storage**, **Delete all comments**, and Help. The sidebar header pins a meta row (document-type bubble, layer version, and the
right-aligned **Help & About** / **Hide** pair), the comments count with **Copy all**, sort arrows, one compact row
for **Generated on** and **Last comment**, an **Export** disclosure holding the file formats (Shareable, Offline,
Plain HTML, Markdown, PDF), and a **More** menu holding **Manage storage** and **Delete all comments**.
...
The document-type bubble reports **Shareable**, **Offline**, or **Not shareable**. Its tooltip explains whether the file
...
The file remains **Not shareable** until **Export as Shareable** writes a fresh copy without that embedded comment.
...
panel and toolbar, Shareable / Not shareable, exporting and sharing, sending comments to an agent, navigation, reading
aids, keyboard and accessibility, self-contained and privacy, and About links.

**Delete all comments** keeps a danger color and opens a confirm dialog whose default action is **Cancel**. Enter, Escape,
or backdrop cancel unless the user explicitly chooses OK. It is reachable from the sidebar **More** menu and from the
collapsed toolbar's **... more** menu; both entry points run the same confirmed flow.
...
- Place the `nav.cm-toc` below the document title (and, on a report or plan, below the `doc_stats` reading-time strip that sits under it), so the reader meets the title and the reading time before the contents list. `generate_toc.py` anchors a GENERATED nav directly under the title (or under that strip), which is above any intro paragraph; if you want the nav below your intro, write it there by hand and do not re-run the generator over it. The generator keeps the nav at the top of `#commentRoot` instead whenever the title container is not entirely above every listed `h2`/`h3` - a slide deck, a document written inside one wrapper element, or a lede that itself carries a listed subtitle heading - and whenever the nav would list nothing at all.
- If your section headings are numbered (for example `1. Goals`, `2. Scope`), do NOT number the TOC entries yourself: `generate_toc.py` (and `finalize.py` / `retrofit.py` for an existing `<ol>` `.cm-toc`) strip the redundant leading number from each entry so it is never double-numbered. Keep the `<ol>`. In a nav the GENERATOR produced, the number is baked into each entry as a `cm-skip` `span.cm-toc-num` and the ordered-list marker is dropped, so the Contents list shows the same hierarchical number the runtime side menu does (`1`, `1.1`, `1.2`, `2`) rather than a flat `1..N` that numbers a subsection as a top-level section. In a nav you write BY HAND the browser's `<ol>` marker still supplies the number, so a hand-written list stays flat; run the generator over it if you want the hierarchical numbering.
