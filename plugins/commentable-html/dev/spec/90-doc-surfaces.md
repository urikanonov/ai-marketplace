...
| CMH-OFFLINE-07 | opt-out: a correctness fix to the already-documented Offline export - re-exporting an offline file now works instead of erroring, so the reader gets the behavior the docs already promise, with no new capability | opt-out: a re-export failure a viewer never saw succeed; there is nothing new to demonstrate on a slide |
