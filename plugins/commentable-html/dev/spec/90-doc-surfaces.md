...
| CMH-A11Y-13 | opt-out: an accessibility correctness invariant on chrome the reviewer already uses - a toast stops hiding behind a dialog scrim and a recovery dialog stops dropping focus on `<body>`; there is no new capability to teach | opt-out: a stacking and focus-restore invariant behind the existing storage-recovery toast, with nothing new for a viewer to see on a slide |
