...
| CMH-BUILD-25 | opt-out: an internal supply-chain guard over the vendored library bytes; a reader of a document sees no new capability, only the same libraries provably at the declared version | opt-out: a provenance guard over build inputs, with nothing to demonstrate on a slide |
