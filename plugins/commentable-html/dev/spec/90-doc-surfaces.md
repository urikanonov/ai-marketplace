...
| CMH-SIZE-09 | opt-out: an example-content and size correction - the same charts render from a pinned CDN loader instead of a bundled copy, so a reader opens the same document, only smaller | opt-out: nothing to demonstrate on a slide; the deck already shows charts, and what changed is where their library comes from |
