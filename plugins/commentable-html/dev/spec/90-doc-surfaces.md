...
| CMH-BUILD-31 | opt-out: internal Playwright test-harness hermeticity (a spec that opens a shipped example serves both vendored libraries locally instead of fetching them); nothing changes in the document a reader opens | opt-out: internal test-harness hermeticity, not a deck topic |
...
| CMH-BUILD-30 | opt-out: maintainer-only build safety that prevents generated output from entering its source tree; no reader-facing behavior changes | opt-out: a build-output placement guard, with nothing a viewer can be shown |
...
| CMH-TOC-12 | help | opt-out: the fold is a flow-document reading affordance for a long report, and a deck gets no in-document navigation chrome at all - a Contents list authored on a slide is left as plain content, so there is nothing to demonstrate on a slide |
