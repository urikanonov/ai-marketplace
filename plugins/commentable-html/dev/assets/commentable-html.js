...
function _cmhRegionMarkerPattern(kind, name) {
  return new RegExp("^[ \\t]*(?:<!--[ \\t]*)?(?:/\\*[ \\t]*)?(?:=+[ \\t]*)?"
    + _cmhEscapeRegExp(kind + ": commentable-html - " + name)
    + "[ \\t]*(?:=+[ \\t]*)?(?:-->|\\*/)?[ \\t]*$", "gm");
}
function _assertSingleRegionMarkers(html, name) {
  const begins = Array.from(String(html || "").matchAll(_cmhRegionMarkerPattern("BEGIN", name)));
  const ends = Array.from(String(html || "").matchAll(_cmhRegionMarkerPattern("END", name)));
...
function _ensureOfflineCsp(doc) {
  ...
  meta.setAttribute("content", "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; font-src data:; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'");
  head.insertBefore(meta, head.firstChild);
}
function _offlineScriptHasNetworkImport(body) {
  const src = String(body || "");
  return (/\bimport\s*\(/.test(src) && /(?:https?:)?\/\//i.test(src)) ||
