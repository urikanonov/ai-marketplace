...
// A <link> fetches through more than its `href`: a preload link carries the URL in `imagesrcset`
// with NO href at all - a `rel=preload as=image` link whose `imagesrcset` names an https host was
// measured requesting it in a real Chromium - and `imagesizes` rides with it. Both were invisible
// to this strip and to the strict validator alike (#999). Mirrored by the validator's
// `LINK_IMAGE_ATTRS`, pinned by `test_the_python_and_js_link_image_attributes_agree`. Written
// without the markup spelled out, because this file's own text is inlined into an export that the
// gate then reads: a literal tag carrying a network URL in a comment is one the tokenizer sees.
const _OFFLINE_LINK_IMAGE_ATTRS = ["imagesrcset", "imagesizes"];
...
function _stripOfflineNetworkLoads(doc) {
  ...
  const clearAttr = function (el, attr) {
    ...
    const network = (attr === "srcset" || attr === "imagesrcset") ? _offlineSrcsetHasNetwork(value) : _offlineIsNetworkUrl(value);
    if (!network) return;
  ...
  // The pass above reads a link's `href`, which is not the only way one fetches: a preload link
  // carries the URL in `imagesrcset` with NO href at all (a `rel=preload as=image` link whose
  // `imagesrcset` names an https host was measured requesting it in a real Chromium), and
  // `imagesizes` rides with it. In an offline export the zero-network CSP still blocks the fetch
  // (`img-src data:`), but this strip is the layer that must not DEPEND on the CSP, and the strict
  // validator was blind to the same attributes. Unlike the `href` pass this clears the ATTRIBUTE
  // rather than removing the element - a `rel`/`as` link is metadata an author may want kept, and
  // there is no load left once the URL is gone - so it is `rel`-blind, exactly as the validator's
  // copy is: a value that names a network URL goes whatever the `rel` says.
  all("link").forEach(function (el) {
    _OFFLINE_LINK_IMAGE_ATTRS.forEach(function (attr) { clearAttr(el, attr); });
  });
