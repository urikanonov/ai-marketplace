...
| `50-sidebar.js` | CMH-SIDE, CMH-PERSIST, CMH-RICH | Sidebar rendering and durable embedded-delete persistence from per-card deletes; owns `cmhConfirmDeleteThread`, the one confirm-and-delete path every surface (the card and the in-document dialog) shares; renders the reviewer note rich, carries the hidden raw-source element, and gives the inline reply/edit editors the shared formatting toolbar + shortcuts. |
