...
- Shared infrastructure used across modules: the content-root boundary helpers `cmhContentRootState` /
  ...
  A third pairing runs that way for the display-zone re-stamp: `50-sidebar.js` calls
  `cmhRefreshCommentPopoverTime` (53-comment-popover) so an open in-document dialog is re-stamped
  with the cards, and `06-preferences.js` calls `cmhApplyTimeZoneChange` (50-sidebar) from
  `setUtcTimes` so the writer itself re-stamps. Both are legal for the same reason - one hoisted
  IIFE scope, called at event time, never at load time - but move them together.

| Module | SPEC areas | Purpose |
| --- | --- | --- |
| `00-preamble.js` | CMH-CORE, CMH-EXP, CMH-CONTENT, CMH-VAL | IIFE opener; captures `SNAPSHOT_HTML` and `document.currentScript` before any DOM access, owns the SINGLE copy of the shared HTML-STRING boundary readings (`_CMH_SPACE_CH`, `_CMH_NAME_END_SRC`/`_CMH_NAME_END_CH`, `_CMH_RAW_TEXT`, `_cmhTagEnd`, `_cmhTagName`, `_cmhCommentEnd`, `_cmhScriptDataClose`, `_cmhRawTextClose`) that `65-export-shareable.js` also walks with - they live HERE, above `SNAPSHOT_HTML`, because the snapshot is serialized through them at LOAD time and a `const` is not hoisted - and the serialized-CR rules built on them (`cmhEscapeSerializedCarriageReturns`, the `cmhEscapeCr` spelling, and `cmhSerializeTextData` for a bare text node - CMH-EXP-24), which `cmhSerializeElement` applies ONCE to the output of every caller (the load-time snapshot captures the RAW assembly first, since it must stay the FIRST statement in the IIFE, and takes the pure pass a few lines later) so an authored CR is written back as `&#13;` instead of a literal CR the next load folds to LF, preserves serializable declarative shadow roots in that snapshot, declares the layer-chrome identity registry (`cmhMarkLayerChrome` / `cmhClickHitsLayerChrome`), and owns the SINGLE copy of the authored-text-preserving slot permutation (`cmhPermuteChildrenInSlots` writer / `cmhPermutedChildNodes` reader, CMH-CONTENT-21) plus `CMH_HASH_EXCLUDED`, the by-construction identity set the content-hash scan honors (kept SEPARATE from the heuristic `CMH_INJECTED_CHROME` export-tail set, where an over-capture is harmless but here would silently subtract document text). Any pass that reorders element children inside `#commentRoot` must go through them - appending instead strands the whitespace between the elements and drifts the content hashes. |
| `01-cold-tier.js` | CMH-COLD | Cold-tier loader: the inlined pure-JS gzip inflate and `cmhHydrateColdTier()`, which `00-preamble.js` calls as its FIRST statement (ahead of the `SNAPSHOT_HTML` capture) so the compressed table rows are materialized before anything - anchors, hashing, search, print, every export - reads the DOM. Uses only hoisted `function` declarations and function-locals, because it runs above every module-level `const`. |
| `01-config.js` | CMH-CORE, CMH-FWDCOMPAT, CMH-DENSITY, CMH-SEC, CMH-EXP | Auto-discovered config; declares `CMH_VERSION` (build.py stamps it), the brand-site link builder (`cmBrandSiteMark`) and the content-root boundary helpers (`cmhContentRootState`, `cmhContentRoot`, `cmhLayerIdOwners`, `cmhLayerBlocks`, `cmhLayerBlock`, `cmhReadLayerBlock`, `cmhWarnUnresolvedBlock`, `cmhWarnAmbiguousBlock`) every reader and exporter resolves the layer's own reserved data blocks through. |
...
| `06-preferences.js` | CMH-MENU-PREF | Scoped reviewer preferences: the cross-document default and the per-document override behind "Auto-open panel on comment", plus the cross-document-only display zone behind "Show times in UTC" (`utcTimesEnabled` / `setUtcTimes`) and the applied-zone shadow state (`cmhUtcTimesChanged` / `cmhMarkUtcTimesApplied`) that keeps a re-stamp to the passes where the zone really moved. Each read/write is behind a try/catch guard. |
...
| `46-comment-selection.js` | CMH-PICK | Per-comment selection: the transient, self-pruning pick set of thread roots, the panel's selection bar, and the confirmed `Delete selected comments` action. |
| `49-head-aux.js` | CMH-RESP, CMH-A11Y | The sidebar header's bounded `.head-aux` scroll region (CMH-RESP-16): stamps a keyboard tab stop, `role="group"` and an accessible name WHILE it actually scrolls, and takes them back when it does not, so its non-focusable metadata lines are reachable without a dead tab stop. |
| `50-sidebar.js` | CMH-SIDE, CMH-PERSIST, CMH-RICH, CMH-PICK | Sidebar rendering and durable embedded-delete persistence from per-card deletes; owns `cmhConfirmDeleteThread`, the one confirm-and-delete path every surface (the card and the in-document dialog) shares; renders the reviewer note rich, carries the hidden raw-source element, renders each card's pick control and its ONE unified action row, and gives the inline reply/edit editors the shared formatting toolbar + shortcuts. It also owns the bundle's SINGLE timestamp formatter and its display-zone infrastructure - `formatTime` / `cmhFormatInstant` / `cmhLocalZoneLabel` / `CMH_TZ_OPTION_HONORED` / `cmhTimeMetaHtml`, plus `cmhGeneratedIso`, `cmhRefreshTimeLabels` and the `cmhApplyTimeZoneChange` re-stamp guard (and the `storage` listener that drives it, registered here rather than in the More menu so it still runs in a document whose COMMENT UI region has no Preferences rows) - which `53`, `55`, `56`, `57`, `83` and `95` all call. |
| `51-comment-search.js` | CMH-SEARCH, CMH-RICH, CMH-PICK | Comment search / filter row: case-insensitive filter of the rendered cards (matching the hidden raw note source so markers/URLs stay searchable), shown/total count, clear button; refreshes the selection bar's hidden-pick disclosure. |
| `52-hover-bubble.js` | CMH-CORE | Hover bubble to open a comment. |
| `53-comment-popover.js` | CMH-CORE, CMH-RICH | Inline on-screen comment dialog opened from the hover bubble (renders the note rich; note + Edit and Delete buttons, the Delete calling the sidebar's shared `cmhConfirmDeleteThread` so both surfaces delete identically; the in-place editor carries the shared formatting toolbar and shortcuts; an outside pointer click closes it, and is swallowed when it lands in the annotated document unless it hits one of the layer's identity-resolved surfaces - an open editor, or a control registered through `cmhMarkLayerChrome` - while a keyboard-activated one is never swallowed). |
| `54-sidebar-toggle.js` | CMH-SIDE, CMH-A11Y | Sidebar open/close. |
| `55-toolbar-menu.js` | CMH-MENU-ICON, CMH-MENU-PREF, CMH-UI | Toolbar overflow menu; renders the menu header's brand-site link and running-version text, plus the matching brand-site link in the floating toolbar left of the overflow trigger; wires the sidebar More menu's Preferences checkbox rows and its roving focus. |
| `56-copy-clear.js` | CMH-COPY, CMH-PICK | Copy all + Clear all, and the selection-scoped `Copy selected` bundle. |
...
| `65-export-shareable.js` | CMH-EXP, CMH-SEC | Export as Shareable + shared export snapshot primitives. Its tag walk (`_cmhForEachTag`) reads the HTML-string boundary readings from `00-preamble.js` rather than keeping a second copy. |
