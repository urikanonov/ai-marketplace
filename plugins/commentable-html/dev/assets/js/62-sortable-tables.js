...
// because it lands in a toast, where an unbounded message would push the actionable sentence away.
...
// and no toast was shown, leaving a reader unable to tell a failed export from a click that never
// registered (#1052). Failure returns null rather than an empty array, so no caller can mistake a
// pass that failed for a document that simply has no comments and export an empty set.
...
// A sort changes the clicked table's TEXT as its ancestors' cells read it, and an outer table sorted
// on a column whose cells HOLD that table was ranked against the OLD order. Nothing re-ranked an
// ancestor when a descendant moved, so a reader who sorted the outer table FIRST and the nested one
// second was left looking at an order the innermost-first load replay never reproduces (#1053). The
// alternative - making an outer sort key ignore nested-table text - was rejected: it would silently
// rank a column that holds nothing but a nested table as empty, which is a worse surprise than the
// replay mismatch. So re-apply each sortable ANCESTOR's persisted sort, nearest first (its own move
// then re-ranks ITS ancestor), which makes the live view the replay's own answer for either click
// order. UNSORT before re-sorting: `_sortRows` breaks ties on a row's CURRENT index and the replay
// always sorts from the authored order, so re-sorting an already-sorted ancestor in place could
// settle ties on an order the replay never reaches.
function _reapplyAncestorSorts(t) {
  let cur = t.parentElement ? t.parentElement.closest("table") : null;
  if (!cur) return;
  const all = _sortableTables();
  // Re-ranking an ancestor permutes the row that HOLDS this table, and the slot swap detaches and
  // re-inserts that row - taking the chevron the reader just pressed with it, which drops focus to
  // <body>. Nothing else here moves the actuated control (a table's own chevrons live in its thead,
  // which is never permuted), so put focus back rather than dumping a keyboard reader mid-sort. The
  // restore deliberately does NOT pass preventScroll: the control physically moved, so following it
  // is the point - pinning the viewport could leave the focused chevron off-screen on a tall table.
  const focused = document.activeElement;
  while (cur) {
    const i = all.indexOf(cur);
    if (i >= 0) {
      const st = _tableSortState[_tableKeyFor(cur, i)];
      // One ancestor that cannot be re-ranked must not swallow the click that reached this handler,
      // nor stop a further-out ancestor being re-ranked. It is left UNSORTED rather than put back:
      // that is exactly what the load replay does with a sort that throws, so the live view still
      // agrees with the order a reload produces.
      if (_validSortState(st)) {
        try {
          const body = _tableBody(cur);
          _unsortRows(body);
          _sortRows(body, st.col, st.dir);
        } catch (e) { /* re-rank the rest */ }
      }
    }
