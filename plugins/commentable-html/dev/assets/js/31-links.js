...
function stampLinkTargets() {
  root.querySelectorAll("a[href]").forEach((a) => {
    ...
    if ((a.getAttribute("target") || "").trim().toLowerCase() === "_blank") {
      ...
      // looked like it already named `noopener`, so nothing was stamped and the browser - which
      // reads ONE opaque relation - left `window.opener` exposed (#1120). The RAW tokens are what
      // is written back, so an author's casing and any relation they authored survive.
      ...
      ["noopener", "noreferrer"].forEach((t) => {
        if (have.indexOf(t) === -1) { raw.push(t); have.push(t); changed = true; }
      });
