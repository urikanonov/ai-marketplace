...
function setupSideToc() {
  ...
  // keystroke. Only the title the menu shows is matched - the entry label plus the number the
  // DOCUMENT itself supplies, never the sequential one the runtime computes when the document has
  // none (that number is chrome, not title text). Body prose is not part of a title, and the review
  // status mark is a CSS pseudo-element, so neither can reach a query. An icon-only author nav link
  // leaves an empty label, so fall back to its heading's own text rather than to nothing - that
  // entry would otherwise be unmatchable by any query.
  items.forEach(function (it) {
    const label = _cmTocNormalize(it.label)
      || _cmTocNormalize(_cmHeadingDepth(it.el) ? cmhHeadingText(it.el) : "");
    it._cmHay = _cmTocNormalize((it.docNum ? it.docNum + " " : "") + label);
  });
