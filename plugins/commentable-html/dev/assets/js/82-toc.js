...
function _cmTocOwnText(root, items) {
  ...
  (function walk(node) {
    for (let n = node.firstChild; n; n = n.nextSibling) {
      ...
      // Foreign-namespace elements (an inline SVG's <style>/<script>) report a lowercase name, so
      // read localName and compare case-insensitively rather than trusting HTML's uppercase tagName.
      if (/^(script|style|template)$/.test((n.localName || n.tagName || "").toLowerCase())) continue;
  ...
  return items.map(function (it, i) {
    // An author TOC may point outside the content root, which this walk never visits; fall back to
    // that element's own text so such an entry still filters on its body rather than its label alone.
    if (!bufs[i].length && it.el && !root.contains(it.el)) return it.el.textContent || "";
    return bufs[i].join("");
  });
}
