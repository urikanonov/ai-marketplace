...
function _clipContainerFor(node) {
  ...
  // A direct-child pre.mermaid gallery card is already covered by the bare `pre.mermaid` match; add the
  // other two gallery card types (div.mermaid, bare figure) scoped to a gallery, rather than a broad
  // `.cmh-diagram-gallery > *` that would treat any stray direct child as a clip container.
  return el && el.closest
    ? el.closest("pre.mermaid, figure.chart, table, .cmh-diff-raw, .cmh-diagram-gallery > div.mermaid, .cmh-diagram-gallery > figure")
    : null;
}
...
function markGalleryCardScrollable(host) {
  ...
  if (!card) return;
  const overflows = card.scrollHeight > card.clientHeight + 1 ||
