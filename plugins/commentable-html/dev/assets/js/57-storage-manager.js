...
function _cmhTombstoneForeign(base, ids) {
  ...
  try {
    ...
    const set = new Set(existing.filter(function (x) { return typeof x === "string" && SAFE_ID_RE.test(x); }));
    ids.forEach(function (x) { set.add(x); });
    localStorage.setItem(delKey, JSON.stringify(Array.from(set).slice(0, CMH_MAX_COMMENTS)));
