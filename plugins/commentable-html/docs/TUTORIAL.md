...
4. An inline `<svg>` figure works the same way: hover it (or focus it and press Enter) and click **Add Comment**. The comment quotes the graphic's `aria-labelledby`, `aria-label` or direct-child `<title>`.
...
2. The nav has a search box at the top: type to filter the section list down to the headings whose title matches (it matches section titles only, not their body text, and ignores letter case).
...
4. The **More** menu also holds **Delete all comments**, which removes every comment (after a confirmation) so you can start the review over.
5. Every time the panel shows - a comment's timestamp, **Generated on**, **Last comment**, the footer, the Copy all bundle, and the printed appendix - names the timezone it is in, so two reviewers in different places read the same comment the same way. Under **More > Preferences**, **Show times in UTC** switches all of them to UTC instead of your computer's local zone; it applies straight away, with no reload, and is your setting for every commentable-html document in this browser.
