...
function _offlineIsSameOriginUrl(v) {
  const raw = String(v || "").trim();
  const absolute = raw.indexOf("//") === 0 ? location.protocol + raw : raw;
  if (!/^https?:\/\//i.test(absolute) || !/^https?:/i.test(location.protocol)) return false;
  try { return new URL(absolute, location.href).origin === location.origin; } catch (e) { return false; }
}
function _offlineIsExternalNetworkUrl(v) {
  return _offlineIsNetworkUrl(v) && !_offlineIsSameOriginUrl(v);
}
function _offlineSrcsetHasExternalNetwork(v) {
  return String(v || "").split(",").some(function (part) {
    return _offlineIsExternalNetworkUrl(part.trim().split(/\s+/)[0]);
  });
}
...
function _stripOfflineNetworkLoads(doc) {
  ...
  const clearAttr = function (el, attr) {
    ...
    const external = attr === "srcset" ? _offlineSrcsetHasExternalNetwork(value) : _offlineIsExternalNetworkUrl(value);
    if (!external) return;
