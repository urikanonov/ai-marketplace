# Charts with tooltips (Chart.js)

Reports rendered as commentable HTML frequently need real charts: line trends, bar
comparisons, dual-axis, stacked. Hand-rolled inline SVG is painful the moment you want
hover tooltips, shared multi-series readouts, log scales, or a legend. Use **Chart.js**
(canvas based) instead. It ships professional tooltips, legends, axes, and animations
...
document so that (a) the tooltips work, and (b) the chart coexists cleanly with the
commenting layer. It is light-theme by default (the house style); a dark-theme note is at
the end.
...
- **One dependency, graceful offline.** A single `<script src>` tag. If the CDN is blocked or
  offline, the canvas stays blank and the rest of the document (including comments) still works.
...
2. **Inject chart scripts before the FINAL `</body>`, never the first.** `TEMPLATE.html`
...
3. **Load Chart.js in `<head>` (or before the init script) with a plain SYNCHRONOUS tag.** Do not
   ...
   init and un-deferred. For a self-contained shareable artifact, pin the version and add a
   Subresource Integrity hash + `crossorigin` so a compromised CDN cannot inject code into a file
   ...
   `curl -s <url> | openssl dgst -sha384 -binary | openssl base64 -A`.) Guard the init with
   `if (typeof Chart === "undefined") return;` so an offline / CDN-blocked / SRI-mismatch load
   degrades to a blank canvas instead of a thrown error. If the report will be served under a
...
(function () {
  ...
  if (!el || typeof Chart === "undefined") return;            // offline / CDN blocked: no-op
...
3. **Automated (static, no browser).** Run `python tools/validate.py <file.html>` (ships with this
   ...
   + the `typeof Chart` offline guard. Use `--charts-only` to run just the chart checks.
