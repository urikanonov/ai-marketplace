...
**Trust boundary (MUST).** The content you place inside `#commentRoot` (including anything passed to `new_document.py --content`) is treated as **trusted HTML** and is emitted verbatim - the tools and the runtime do **not** sanitize it. The runtime only protects reviewer-supplied data: it escapes or `textContent`s all comment text and metadata, validates comment ids against `SAFE_ID_RE`, and escapes `<` as `\u003c` in the `embeddedComments` JSON. Scripts, event handlers, and `javascript:`/`data:` URLs in the authored content are NOT neutralized, and the validator does not make arbitrary host HTML safe. If any part of the content originates from an untrusted source, sanitize it yourself before wrapping it.
