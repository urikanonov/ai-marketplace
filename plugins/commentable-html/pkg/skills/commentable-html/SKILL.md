...
**SHOULD** first pick the output mode (see "Output modes: standalone vs economy"). Default to **standalone** (`TEMPLATE.html`, one self-contained file). Choose **economy** (copy `dist/` companions + start from `dist/ECONOMY.html`) only for local, iterated documents where regeneration cost matters and the file will stay next to its companions.

**For a NEW standalone document, MUST use `tools/new_document.py`** when Python is available. It clones `TEMPLATE.html`, drops in only your content fragment, sets the `#commentRoot` data attributes, refuses demo keys, and self-validates before writing. Do NOT hand-copy the five regions for a fresh document - that is the highest-token, most error-prone path and it walks straight into the duplicate-root footgun described in Step 3.
...
**MUST**, when Python is available, generate rich content artifacts with the tools below instead of hand-authoring them. The AI cannot reliably escape HTML, base64+gzip a Kusto link, or hand-build an offline-guarded chart loader; hand-authoring wastes tokens and causes validator failures. Use the tool every time the artifact appears:

- KQL block + Run-in-Kusto link: `tools/kql_highlight.py` (never build the ADX deep link by hand; `tools/kusto_link.py` builds a bare link).
- Unified code diff: `tools/diff_block.py` (emits the escaped `pre.cmh-diff`).
- Highlighted source code: `tools/highlight_code.py`.
- Chart: `tools/chart_block.py` (emits the full `figure.chart` + the offline-guarded loader + init from a Chart.js spec JSON).
...
**MUST** also run the automated validator before handoff when Python is available: `python tools/validate.py --strict <file.html>`. `--strict` treats every warning as a failure, so one run surfaces *all* issues at once (missing offline inlining, un-escaped diffs, broken Kusto links, duplicate heading ids, unlabeled canvases, and more) - iterate until it reports `OK (0 warning(s))`. Do not skip it and do not hand off with outstanding warnings. It also runs as the final step of `finalize.py`. It codifies the structural invariants (regions, `#commentRoot` wiring, JSON script blocks, escaped `</script>`, required ids, the offline guarantee) and fails two retrofit-specific mistakes: the demo content root surviving the swap (the active `#commentRoot` still carries `data-comment-key="commentable-html-demo-v1"` while the `<title>` was customized) and a real content root left buried inside an HTML comment.
...
This is the near-zero-token iteration step - it never rewrites the document body, which matters most in economy mode where the boilerplate lives in companion files.
...
## Output modes: standalone (inline) vs economy
...
| | **Standalone (inline)** - `TEMPLATE.html` | **Economy** - `dist/ECONOMY.html` + companions |
|---|---|---|
...
**Default to standalone.** Economy is opt-in. A loose economy HTML opened where it cannot reach the assets is broken, so only choose economy when the report can reach the skill's `dist/` folder at a stable path (local iteration) and the user has asked to optimize regeneration cost.

### Producing an economy document
...
2. Use `dist/ECONOMY.html` as the starting shell (head `<link>` + version `<meta>`, the ECONOMY BOOTSTRAP banner, the inline state/UI regions, and the two `<script src>` companions at the end of `<body>`); repoint the `href`/`src` at the skill `dist/` path. Replace the demo content inside the CONTENT markers with your own; set a unique `data-comment-key`.
3. Keep the version `<meta name="commentable-html-assets" content="<V>">` matching the referenced asset version.

### Guardrails that make economy safe (built in)
...
- **Missing-asset banner.** The ECONOMY BOOTSTRAP block reveals a visible red banner if the companion runtime does not initialize within 3 s, so a broken share never fails silently.
- **Export as Portable embeds everything.** In economy mode the export action is labeled **Export as Portable**: it rebuilds ONE self-contained inline file - reading the CSS/JS string payloads from the loaded `.assets.js` registry (works from `file://` without `fetch`), inlining them, and embedding the current comments - so the downloaded file no longer depends on the skill folder or any companion. A user always gets a portable single file, even starting from an economy document. Producing the economy (referenced-asset) variant is the agent's job at generation time, not an in-page conversion.
...
Changing the layer's code, rebuilding `TEMPLATE.html` / `dist/`, and running the Python and browser test suites are maintainer tasks - all of that lives in **`docs/DEVELOPMENT.md`**. Per generated document, the only thing to run is `tools/validate.py` (below).
...
Five drop-in regions, all included in `TEMPLATE.html` and clearly bracketed with `BEGIN: commentable-html v2 - <REGION>` / `END: commentable-html v2 - <REGION>` comments so you can grep them out and paste them as a unit:
...
KQL blocks should pair a commentable `<pre><code class="language-kusto">` with a safe Run in Kusto deep link. The caption title is the click-to-copy cluster affordance, so do not add a separate cluster chip. See [Kusto query blocks](references/kusto-query-blocks.md) for link generation, data-safety rules, and markup.
...
Unified diffs can be rendered as offline, line-commentable review blocks with inline and side-by-side layouts plus an offline **Syntax: on/off** highlighting toggle. See [Code review diffs](references/code-review-diffs.md) for markup, escaping, anchoring, and use cases.

## Charts with tooltips

Chart.js charts are supported when the canvas lives in `figure.chart`, the wrapper is `cm-skip` for text offsets, and init runs after the commentable JS region. The canvas itself is commentable as `imageKind: "chart"`. Load Chart.js from a local vendored or inline asset by default; CDN loading is explicit opt-in and breaks offline operation. See [Charts with tooltips](references/charts-with-tooltips.md) for embedding and verification details.

## Network requirements

The core layer makes no external network calls. Optional external assets are limited to mermaid, Chart.js, and any tooltip library a host page deliberately adds; generated reports should self-host or inline them by default. CDN loading must be called out as an explicit opt-in because it breaks the offline/privacy guarantee.
...
The export action downloads a fresh copy with the current comments written into `#embeddedComments`; in economy mode it also inlines the companion assets into one portable file. See [Exports](references/exports.md) for merge semantics and implementation details.

## Combined file from an economy document

Economy exports inline the loaded CSS and runtime from the asset registry so the downloaded file is portable. See [Exports](references/exports.md) for the exact rebuild behavior.
...
Retrofitting is a marker-by-marker copy from `TEMPLATE.html` plus a `#commentRoot` wrapper and optional `cm-skip` exclusions. See [Retrofitting](references/retrofitting.md) for the full paste order and host-page checklist.
...
It refuses economy documents (companion assets), preserves the document's state/content, and self-validates the result. See [Retrofitting](references/retrofitting.md) for the mechanical upgrade recipe.
...
- **`TEMPLATE.html`** - complete inline template and demo; copy its five regions for standalone mode.
- **`dist/`** - economy shell and companion CSS/JS/assets bundle for local iterative documents.
- **`tools/*`** - deterministic, stdlib-only Python helpers you SHOULD prefer over hand-editing (they remove AI variance and are self-validating). Run any with `python tools/<name>.py --help`:
  ...
  - `upgrade.py <file> [--check]` - swap the CSS/COMMENT UI/JS regions of a deployed file to a newer TEMPLATE, preserving comments/state (validates before it commits; never clobbers on failure).
  ...
  - `chart_block.py --spec <json|-> --canvas-id ID --caption C [--title T]` - emit a validator-clean `figure.chart` + offline-guarded loader + init from a Chart.js spec.
  - `kql_highlight.py` / `kusto_link.py` - build a full `figure.cmh-kql` and a deterministic Run-in-Kusto deep link (never hand-build the link).
  ...
  - `build.py` - (maintainer only) regenerate `TEMPLATE.html` + `dist/` from the source assets.
- **`docs/*`** - maintainer and deep-dive documentation, including chart and development guidance.
...
Run `python tools/validate.py <file.html>` when Python is available to check the structural invariants. See [Validation](references/validation.md) for error, warning, chart, and mutation-test details.
...
Emit **one** fire-and-forget usage event when this skill finishes (exits `0`, never blocks); the CLI session id is auto-detected.

```powershell
python wiki/scripts/telemetry/track_skill_usage.py --skill commentable-html --source skill --success <true|false> --attrs '{"htmlFile":"<file>","regions":"5"}'
```

- If **another skill dispatched this one**, add `--parent-skill <caller>`. If triggered by an on-call **ICM `#copilot` reply**, add `--requested-on-behalf --icm-id <id> --trigger icm-reply`. 🔴 **Never pass a person's alias/name/PII** - `--requested-on-behalf` is a value-less flag that only records *that* the run was requested on someone's behalf (`requestedBy=true`), never who.
- On failure use `--success false --error-type "<short-class>"`.

See [`scripts/telemetry/README.md`](../../scripts/telemetry/README.md) and the global contract in [`AGENTS.MD`](../../../AGENTS.MD) → *Skill Usage Telemetry*.
