...
| UPD-15 | The Claude `SessionStart` group has a bash handler (macOS/Linux) and a powershell handler (Windows); the bash handler no-ops on Windows (`uname` MINGW/MSYS/CYGWIN) so exactly one runs per platform and the update never fires twice. | `updater.Tests.ps1` -> "UPD-15 Claude bash hook handler no-ops on Windows to avoid a double run" |
