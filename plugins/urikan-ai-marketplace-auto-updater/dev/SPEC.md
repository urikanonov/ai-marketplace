...
| UPD-07 | On macOS/Linux, when `pwsh` (PowerShell 7) is missing the bash field writes a dated skip note to the plugin-data log instead of silently no-opping, then succeeds. The missing-pwsh else-branch is EXECUTED under a real POSIX shell (bash on Linux, Git Bash on Windows) against a sandbox `COPILOT_HOME`, asserting the skip note is actually written, not just pattern-matched. | `updater.Tests.ps1` -> "UPD-07 missing-pwsh log signal" |
