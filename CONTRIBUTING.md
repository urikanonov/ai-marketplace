...
- What must be green to merge: the required checks `validate` (schema, script unit tests, Markdown, changelog sync, and a secret-bearing-file guard), `version-bump` (a shipped-source change needs a version bump), `build-check` (the commentable-html layer's committed `dist/` matches its `dev/` source), `summary` (the `plugin-tests` gate), and `require-owner-approval` (external PRs carry the maintainer's approving review). The site `build` job runs on pull requests that touch the site, but it is path-filtered and is not a blanket required check. New plugins are maintainer-authored (see [MAINTAINING.md](MAINTAINING.md)).
