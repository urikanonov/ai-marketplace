...
- Add browser tests to a plugin: add `plugins/<plugin>/dev/package.json` (with `@playwright/test`),
  `playwright.config.js` (with at least a `fast` project - the `playwright` job runs `--project=fast`;
  see "Testing in CI" for the fast/heavy split), and specs under `dev/tests/`; `plugin-tests.yml` runs
  them automatically (see `plugins/commentable-html/dev/`). Everything under `dev/` stays in the repo but is never distributed.
