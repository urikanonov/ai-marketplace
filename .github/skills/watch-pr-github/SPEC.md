...
| WPG-DECISION-20 | `Test-ViewerApproved` decides whether the viewer's own review currently approves the PR from their LATEST meaningful review (APPROVED / CHANGES_REQUESTED / DISMISSED; a later COMMENTED review does not revoke a standing approval), so a stale earlier APPROVED does not count after the viewer later requested changes or dismissed their own review. | `decision.Tests.ps1` -> "WPG-DECISION-20 Test-ViewerApproved reflects the viewer's LATEST review state" |
...
`Test-CommenterTrusted`, injecting only the live permission lookup). The shell is kept thin on purpose
so that what is untested is only glue, not logic.
